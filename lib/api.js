...
const CONFIG_KEYS = new Set(["time", "enabled", "broadcast", "digestItems", "footer", "llmProvider", "llmModel", "llmMaxTokens", "targets"]);
...
export function registerApi(ctx, deps) {
	const { getLive, runNow, runState, updateConfig } = deps;
	...
	const statusView = () => {
		...
		return {
			...
			today: outbox.date || sent.last_sent_date,
