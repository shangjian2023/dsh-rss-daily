...
import { runOnce, getStatus, redeliver } from "./pipeline.js";
...
function normalize(config) {
	...
	c.language = c.language === "en" ? "en" : "zh";
	c.timezone = /^UTC([+-]\d{1,2})(:\d{2})?$/.test(String(c.timezone || "")) ? c.timezone : "UTC+8";
	return c;
}
...
const Config = z ? z.object({
	...
	language: z.string().default("zh").description("Digest language: zh | en (one-liners and title follow)."),
	timezone: z.string().default("UTC+8").description("Timezone for digest dates, e.g. UTC+8, UTC-5, UTC+5:30."),
...
function apply(ctx, config) {
	...
	try {
		...
		redeliver: (opts) => redeliver(live, opts) });
	} catch (e) {
			...
			try {
				...
				agent.ctx.effect(() => agent.ctx.tools.register(defineTool({
					...
					async execute(args) {
						...
						if (action === "deliver") {
							// 只重投"今天"的 outbox;陈旧箱报错引导先 run
							const r = await redeliver(live, {});
							if (r.error) return r.error;
							return JSON.stringify({ delivery: r.delivery.results, digest: r.digest }, null, 2);
						}
						if (action === "redo") {
							// 先查再删:运行中删 outbox 会把在途那轮的产物抽走,造成投递了却无法确认
							if (runState.running) return "A run is already in progress; retry when it finishes.";
						...
						if (result.skipped && result.reason === "locked") return "Another pipeline instance holds the lock; try again shortly.";
