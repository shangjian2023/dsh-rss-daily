...
.drd-line{color:var(--dsw-alias-label-primary);font-size:13px;line-height:22px;min-width:0;flex:1}
...
/* ── 对话内插播(portal 进聊天滚动区;样式复刻宿主 assistant 消息:
   无边框文本流 + markdown 排版,规则抄自 ui-renderer 的 _markdown_ 族,
   全部走 --dsw-* theme 变量,与模型回答观感一致) ── */
.drd-bwrap{width:min(var(--dsh-chat-content-width,748px),100%);box-sizing:border-box;margin:16px auto 0;padding:0 16px;animation:drd-in .35s ease}
...
.drd-md{min-width:0;overflow-wrap:anywhere;font:var(--dsw-font-markdown-base);color:var(--dsw-alias-label-primary)}
.drd-md strong{font-weight:600}
.drd-md h3{font:var(--dsw-font-markdown-h3);margin:0 0 16px}
.drd-md p{margin:16px 0 0}
.drd-md p.drd-mdFirst{margin-top:0}
.drd-md a{color:var(--dsw-alias-state-business-primary);position:relative;text-decoration:none}
.drd-md a:hover,.drd-md a:focus{outline:none;text-decoration:underline var(--dsw-alias-state-business-primary)}
.drd-md ol{margin:16px 0 0;padding-left:18px}
.drd-md li{margin-top:6px}
.drd-md li:first-child{margin-top:0}
.drd-md li::marker{line-height:28px;color:var(--dsw-alias-label-secondary)}
.drd-msgMeta{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px;margin-top:10px;display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.drd-msgAct{cursor:pointer;background:0 0;border:0;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px;padding:0;font-family:inherit}
.drd-msgAct:hover{color:var(--dsw-alias-label-secondary);text-decoration:underline}
...
`;
		...
		/* ── API ── */
		const api = {
			get: (p) => fetch("/rss-daily/api/" + p).then((r) => r.json()),
		...
		function Panel({ open, tab, onClose }) {
			...
			const run = async (redo) => {
				...
				else setSt((s) => (s ? { ...s, running: true, phase: "start" } : s));
			};
			...
			return h("div", { className: "drd-backdrop", onClick: (e) => { if (e.target === e.currentTarget) onClose(); } },
				h("div", { className: "drd-modal", onClick: (e) => e.stopPropagation() },
					h("div", { className: "drd-modalHead" },
						h("div", { className: "drd-modalTitle" }, "📰 " + t.title),
						st && !st.running && st.config
							? h("span", { className: "drd-hint" }, fmt(t.dailyAt, { time: st.config.time }))
							: null,
						h("button", { className: "drd-btn", onClick: onClose, "aria-label": t.close }, "✕"),
					),
				...
				h("div", { className: "drd-row", style: { marginBottom: 12 } },
					st?.running
						...
						: h(React.Fragment, null,
							...
							st?.confirmed ? h("span", { className: "drd-ok" }, "✓ " + t.sent)
				...
				adding ? h("div", { className: "drd-card drd-grid" },
					...
					h("div", { className: "drd-field", style: { gridColumn: "1 / -1" } }, h("span", { className: "drd-label" }, t.url),
						h("div", { className: "drd-row" },
							...
							h("button", { className: "drd-act drd-actPri", style: { flex: "none" }, onClick: add }, "＋"))),
				) : null,
		...
		function SettingsForm({ st }) {
			...
			const [err, setErr] = useState("");
			useEffect(() => { if (st?.config && !draft) setDraft(JSON.parse(JSON.stringify(st.config))); }, [st, draft]);
			...
			return h(React.Fragment, null,
				...
				h("div", { className: "drd-row", style: { margin: "6px 0 8px" } },
					...
					h("span", { style: { flex: 1 } }),
					h("button", { className: "drd-act", onClick: () => setDraft({ ...draft, targets: [...draft.targets, { type: "serverchan", key: "" }] }) }, "＋ " + t.addTarget)),
				...
				h("div", { className: "drd-row", style: { marginTop: 12 } },
					...
					msg ? h("span", { className: "drd-ok" }, "✓ " + msg) : null,
		...
		const localToday = () => new Date().toLocaleDateString("sv-SE"); // YYYY-MM-DD,本地时区
		...
		function BroadcastHost() {
			...
			// 在聊天滚动区里安一个自己的 div(会话内容与输入区之间),卸载时移除。
			// 挂载点必须是"有内容的会话":空白新会话不插播,但用户一发消息或切到
			// 历史会话就要能补挂 → 持续慢轮询找位;宿主 div 随会话切换失联则重挂。
			useEffect(() => {
				if (!createPortal) return;
				let stop = false, timer = null;
				const find = () => {
					if (stop) return;
					if (divRef.current && !divRef.current.isConnected) { divRef.current = null; setHost(null); }
					if (!divRef.current) {
			...
			const ready = !!(st && st.items && st.items.length > 0 && st.digest && st.today === localToday());
			...
			if (st?.running) {
				...
				content = h(BcastCard, { st, onDismiss: dismiss });
			}
		...
		function SettingsCard() {
			...
			useEffect(() => {
				...
				const timer = setInterval(() => api.get("status").then(setSt).catch(() => { }), 30000);
				return () => clearInterval(timer);
			}, []);
			return h("div", { className: "drd-card", style: { marginTop: 8 } },
				h("div", { className: "drd-modalTitle", style: { marginBottom: 8 } }, "📰 " + t.title),
				h(SettingsForm, { st }));
		}
