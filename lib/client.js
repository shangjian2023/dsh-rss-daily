...
`;
		...
		const t = zh ? {
			...
			language: "日报语言", timezone: "日报时区", zhLang: "中文", enLang: "English",
			lastDelivery: "上次投递", retryFailed: "重投失败目标",
			exportBtn: "导出", importBtn: "导入",
			importConfirm: "导入将替换现有 {n} 个源,继续?", importBad: "文件格式不对:需要 {\"sources\":[…]} JSON",
			onbTitle: "三步开启每日要闻", onb1: "1. 在「设置」里加一个投递目标(Server酱/TG/Bark…)",
			onb2: "2. 点「获取今日日报」立即试跑一次", onb3: "3. 之后每天这个点自动生成并送达", onbBtn: "去设置",
		} : {
			...
			language: "Digest language", timezone: "Digest timezone", zhLang: "中文", enLang: "English",
			lastDelivery: "Last delivery", retryFailed: "Retry failed targets",
			exportBtn: "Export", importBtn: "Import",
			importConfirm: "Import will replace the current {n} sources, continue?", importBad: "Bad file: expected {\"sources\":[…]} JSON",
			onbTitle: "Daily digest in 3 steps", onb1: "1. Add a delivery target in Settings (ServerChan/TG/Bark…)",
			onb2: "2. Click \"Get today's digest\" for a test run", onb3: "3. It generates and delivers daily from then on", onbBtn: "Open settings",
		};
		...
		const TAG_COLORS = {
			...
			Tech: "#06b6d4", Science: "#8b5cf6", World: "#f97316", Finance: "#d97706",
			Culture: "#a16207", Dev: "#16a34a", Health: "#e5484d", Environment: "#059669",
			Society: "#6b7280", Business: "#6366f1", Product: "#ec4899", Research: "#4f46e5",
		};
		const tagStyle = (tag) => {
			...
			return h("div", { className: "drd-backdrop", onClick: (e) => { if (e.target === e.currentTarget) onClose(); } },
				h("div", { className: "drd-modal", role: "dialog", "aria-modal": "true", onClick: (e) => e.stopPropagation() },
					...
					h("div", { className: "drd-body" },
						cur === "digest" ? h(DigestTab, { st, err, phaseText, onRun: run, onTab: setCur }) : null,
		...
		function DigestTab({ st, err, phaseText, onRun, onTab }) {
			...
			return h(React.Fragment, null,
				...
				st && st.config && (st.config.targets || []).length === 0 && !st.running
					? h("div", { className: "drd-card" },
						h("div", { className: "drd-digestTitle" }, t.onbTitle),
						h("div", { className: "drd-hint", style: { lineHeight: "22px" } },
							t.onb1, h("br"), t.onb2, h("br"), t.onb3),
						h("div", { style: { marginTop: 10 } },
							h("button", { className: "drd-act drd-actPri", onClick: () => onTab && onTab("settings") }, t.onbBtn)))
					: null,
				),
		...
		function SourcesTab() {
			...
			const doExport = async () => {
				try {
					...
					const blob = new Blob([JSON.stringify({ sources: data.sources || [] }, null, 2)], { type: "application/json" });
					const a = document.createElement("a");
					a.href = URL.createObjectURL(blob);
					a.download = "dsh-rss-daily-sources.json";
					a.click();
			...
			const doImport = (ev) => {
				...
				rd.onload = () => {
					try {
						...
						if (!Array.isArray(sources) || !sources.length) throw new Error("empty");
						if (!window.confirm(fmt(t.importConfirm, { n: list ? list.length : 0 }))) return;
						save(sources);
					} catch (e) { setErr(t.importBad); }
				};
				rd.readAsText(f);
			};
			...
			return h(React.Fragment, null,
				h("div", { className: "drd-row", style: { marginBottom: 8 } },
					...
					h("button", { className: "drd-act", onClick: () => setAdding(!adding) }, t.addSource),
						h("button", { className: "drd-act", onClick: doExport }, t.exportBtn),
						h("button", { className: "drd-act", onClick: () => impRef.current && impRef.current.click() }, t.importBtn),
						h("input", { type: "file", accept: ".json,application/json", style: { display: "none" }, ref: impRef, onChange: doImport })),
				err ? h("div", { className: "drd-err" }, err) : null,
		...
		function SettingsForm({ st }) {
			...
			const retryFailed = async () => {
				...
				try {
					const r = await api.send("redeliver", { onlyFailed: true });
					if (!r.ok) setErr(r.data?.error || "request failed");
					else setMsg(t.saved);
				} catch (e) { setErr(String(e)); }
				setBusy(false);
			...
			return h(React.Fragment, null,
				h("div", { className: "drd-grid" },
					...
					h("div", { className: "drd-field" }, h("span", { className: "drd-label" }, t.language),
						h("select", { className: "drd-input", value: draft.language || "zh", onChange: (e) => set("language", e.target.value) },
							h("option", { value: "zh" }, t.zhLang), h("option", { value: "en" }, t.enLang))),
					h("div", { className: "drd-field" }, h("span", { className: "drd-label" }, t.timezone),
						h("input", { className: "drd-input", value: draft.timezone || "UTC+8", placeholder: "UTC+8", onChange: (e) => set("timezone", e.target.value) })),
				...
				st && st.delivery && Array.isArray(st.delivery.results)
					? h("div", { className: "drd-hint", style: { margin: "6px 0" } },
						t.lastDelivery + " " + String(st.delivery.at || "").slice(11, 16) + " · "
							+ st.delivery.results.filter((r) => r.ok).length + "/" + st.delivery.results.length + " ok",
						st.delivery.results.some((r) => !r.ok)
							? h("button", { className: "drd-act", style: { marginLeft: 8 }, disabled: busy, onClick: retryFailed }, t.retryFailed)
							: null)
					: null,
				...
				draft.targets.map((tg, i) => {
					const dres = st?.delivery?.results?.[i];
					const mark = dres && dres.type === tg.type
						? h("span", { className: dres.ok ? "drd-ok" : "drd-err", style: { margin: 0 } },
							(dres.ok ? "✓" : "✗") + (dres.attempts > 1 ? String(dres.attempts) : ""))
						: null;
					return h("div", { key: i, className: "drd-trow" },
